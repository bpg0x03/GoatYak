...
export async function returnFeed(msg: { count: number }, socket: Socket) {
    ...
    //msg = JSON.parse(msg)
    try {
        const items = await Post.find().sort({ updatedAt: -1 }).limit(msg.count).exec();
